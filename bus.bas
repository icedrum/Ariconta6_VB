...
Public Ampliacion As String  'para asientos predefinidos
Public nDocumento As String    'para sientos predefinidos
...
'Para NO leer cada vez, solo una con al inicio
Public CadenaTipoRetencion As String
...
Public Saldo473en_470 As Boolean
...
'Constantes colores SII
Public Const vbSiiOK = &HC0FFC0
Public Const vbSiiMofificando = &HBACD32
Public Const vbSiiEnProceso = &H80FF&
Public Const vbSiiAceptadaConErr = &H17EFF4
Public Const vbSiiError = &HFF&
...
Public Function EstaLaCuentaBloqueada2(ByRef codmacta As String, Fecha As Date) As Boolean
Dim C As String

        EstaLaCuentaBloqueada2 = False
        C = DevuelveDesdeBD("fecbloq", "cuentas", "codmacta", codmacta, "T")
        If C <> "" Then
            'La cuenta esta con fecha de bloqueo
            If Fecha >= CDate(C) Then EstaLaCuentaBloqueada2 = True
        End If
End Function
...
Public Function ComprobarIBANCuentaBancaria2(CuentaCCC As String, cadMen As String) As Boolean
Dim Cta As String
...
Dim K As Integer
    ...
    If CuentaCCC = "" Then
        cadMen = "IBAN vacio."
        ComprobarIBANCuentaBancaria2 = False
        Exit Function
    End If
    
    'Agosto 2020
    'Los dos primeros deben ser TEXTo
    K = CStr(Asc(Mid(UCase(CuentaCCC), 1, 1)))
    If K >= 65 And K <= 90 Then
        K = CStr(Asc(Mid(UCase(CuentaCCC), 1, 1)))
         If Not (K >= 65 And K <= 90) Then K = -1
    Else
        K = -1
    End If
    
    If K < 0 Then
        cadMen = "IBAN debe comenzar letras pais: " & CuentaCCC
        B = False
    Else
    
        If Mid(UCase(CuentaCCC), 1, 2) <> "ES" Then
            'Todo bien. IBAN no nacional
            'S top
        Else
            If Len(CuentaCCC) <> 24 Then
                cadMen = "IBAN no tiene longitud correcta. " & CuentaCCC
                B = False
            Else
        ...
        End If
    End If
    ComprobarIBANCuentaBancaria2 = B
End Function
...
'para que no tenga que ir cadea vez que ve una cuenta a la tabla de tipo reten
Public Sub FijarCadenaTipoRetencion()

    
    
    
    Set miRsAux = New ADODB.Recordset
    CadenaTipoRetencion = "SELECT * FROM usuarios.wtiporeten ORDER BY codigo"
    K = 0
    miRsAux.Open CadenaTipoRetencion, Conn, adOpenForwardOnly, adLockPessimistic, adCmdText
    CadenaTipoRetencion = ""
    While Not miRsAux.EOF
        CadenaTipoRetencion = CadenaTipoRetencion & Format(miRsAux!Codigo, "000") & " " & miRsAux!Descripcion & "|"
        miRsAux.MoveNext
        K = K + 1
    Wend
    miRsAux.Close
    ...
    CadenaTipoRetencion = K & "|" & CadenaTipoRetencion
    
End Sub

Public Function DevuelveEmail(Valor As String, DesdeCodmacta As Boolean) As String
Dim Cadena As String

    Cadena = "codmacta"
    If Not DesdeCodmacta Then Cadena = "nifdatos"
        
    Cadena = Cadena & " = " & DBSet(Valor, "T")
    Cadena = Cadena & " AND maidatos<>'' AND 1"
        
    DevuelveEmail = DevuelveDesdeBD("maidatos", "cuentas", Cadena, "1")
...
Public Sub CargaSufijos(ByRef cboSufijos As ComboBox, CtaBanco As String, SufijoPPal As String)
    Set miRsAux = New ADODB.Recordset
    cboSufijos.Clear
    cboSufijos.ForeColor = vbBlack
    If SufijoPPal <> "" Then cboSufijos.AddItem SufijoPPal
    
    miRsAux.Open "Select * FROM bancos_sufijos WHERE codmacta = " & DBSet(CtaBanco, "T") & " ORDER BY sufijoem", Conn, adOpenForwardOnly, adLockOptimistic
    While Not miRsAux.EOF
        cboSufijos.AddItem miRsAux!sufijoem & " - " & miRsAux!Descripcion
        miRsAux.MoveNext
    Wend
    miRsAux.Close
    Set miRsAux = Nothing
    
    If cboSufijos.ListCount = 0 Then
        cboSufijos.AddItem "--- No hay sufijos disponibles "
        cboSufijos.ForeColor = &HC0C0C0
    End If
End Sub
