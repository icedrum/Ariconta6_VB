...
Begin VB.Form frmInmoUbicacion 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Inmov.  UBICACIONES"
   ClientHeight    =   9390
   ClientLeft      =   45
   ClientTop       =   30
   ClientWidth     =   6690
   Icon            =   "frmInmoUbicacion.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   9390
   ...
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   30
      TabIndex        =   1
      Tag             =   "Nombre|T|N|||inmovubicacion|nomubiin|||"
      Text            =   "Dato2"
      Top             =   8520
      Width           =   1395
   End
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   3
      TabIndex        =   0
      Tag             =   "Código|N|N|0||inmovubicacion|codubiin|000|S|"
      Text            =   "Dat"
      Top             =   8520
      Width           =   800
   End
   Begin MSDataGridLib.DataGrid DataGrid1 
      Bindings        =   "frmInmoUbicacion.frx":000C
      Height          =   7800
      Left            =   150
      TabIndex        =   7
      Top             =   840
   ...
   End
End
Attribute VB_Name = "frmInmoUbicacion"
...
Private Const IdPrograma = 503   'el mismo que inmovilziad
...
Dim Sql As String
    ...
    '### a mano
    Sql = "Seguro que desea eliminar la ubicacion:"
    Sql = Sql & vbCrLf & "Código: " & adodc1.Recordset.Fields(0)
    Sql = Sql & vbCrLf & "Descripcion: " & adodc1.Recordset.Fields(1)
    If MsgBox(Sql, vbQuestion + vbYesNoCancel) = vbYes Then
    
        'Hay que eliminar
        Sql = "Delete from inmovubicacion where codubiin=" & adodc1.Recordset!codubiin
        Conn.Execute Sql
        CargaGrid ""
...
Private Sub Form_Load()
    ...
    'Cadena consulta
    CadenaConsulta = "Select codubiin, nomubiin from inmovubicacion"
    CargaGrid
End Sub
...
Private Function SugerirCodigoSiguiente() As String
    ...
    Sql = "Select Max(codubiin) from inmovubicacion"
...
If Modo = 3 Then
    'Estamos insertando
     Datos = DevuelveDesdeBD("codubiin", "inmovubicacion", "codubiin", txtAux(0).Text, "N")
     If Datos <> "" Then
        MsgBox "Ya existe la ubicicacion : " & txtAux(0).Text, vbExclamation
        B = False
    End If
End If
DatosOK = B
...
Dim Sql As String
    SepuedeBorrar = False
    Sql = DevuelveDesdeBD("codinmov", "inmovele", "ubicacion", adodc1.Recordset!codubiin, "N")
    If Sql <> "" Then
        MsgBox "Existen elementos de inmovilizado en esta ubicacion", vbExclamation
        Exit Function
    End If
...
Dim cad As String
    ...
    If Not Rs.EOF Then
        Toolbar1.Buttons(1).Enabled = DBLet(Rs!CrearEliminar, "N") And Modo = 0
        Toolbar1.Buttons(2).Enabled = DBLet(Rs!Modificar, "N") And Modo = 0
        Toolbar1.Buttons(3).Enabled = DBLet(Rs!CrearEliminar, "N") And Modo = 0
